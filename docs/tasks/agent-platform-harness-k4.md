# Knowledge UI and grounding journey

Beads issue: `agent-platform-harness-k4`. Status and scheduling authority: Beads.

Parent: `agent-platform-research-tools`. Proposal label: K4.

## Requirements

Import/status/source list, context-used display, citations and reviewed-memory distinction; answer remains grounded; refresh/delete change future retrieval.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-k2`, `agent-platform-context-optimisation`, `agent-platform-harness-v4`.
...
Import/status/source list, context-used display, citations and reviewed-memory distinction; answer remains grounded; refresh/delete change future retrieval. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
