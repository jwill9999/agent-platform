...
Beads issue: `agent-platform-harness-k2`. Status and scheduling authority: Beads.

Parent: `agent-platform-research-tools`. Proposal label: K2.

## Requirements

Existing lexical vs SQLite FTS5 evaluation on a small declared corpus; bounded passages; exact citation/version receipts; cross-scope exclusion, stale source and deletion tests. No vector service unless evidence justifies.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-k1`.

Downstream new records: `agent-platform-harness-k4`.
...
Existing lexical vs SQLite FTS5 evaluation on a small declared corpus; bounded passages; exact citation/version receipts; cross-scope exclusion, stale source and deletion tests. No vector service unless evidence justifies. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
