# Persistent state and upgrade policy

Beads issue: `agent-platform-harness-r2`. Status and scheduling authority: Beads.

Parent: `agent-platform-harness-reliability`. Proposal label: R2.

## Requirements

Secret-free state; schema/config/tool revisions; persistent saver; legacy approvals explicitly resolved; drain-before-incompatible-upgrade behavior; restart loads intended state.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-r1`.

Downstream new records: `agent-platform-harness-r3`.
...
Secret-free state; schema/config/tool revisions; persistent saver; legacy approvals explicitly resolved; drain-before-incompatible-upgrade behavior; restart loads intended state. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
