# Verify common coding tasks through the real model-to-tool loop

Beads: `agent-platform-harness-baseline-t1`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix T1: Can the coding agent find/read code, edit a file, run a check and inspect the change?

One disposable Project Chat journey, supported by integration tests

Record tool definitions actually sent to the provider, selected names/arguments, returned results, test exit outcome and file diff. Each task has an available permitted capability; final answer matches observed results. Fixed runner effects do not prove real shell/VM execution.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 4 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
