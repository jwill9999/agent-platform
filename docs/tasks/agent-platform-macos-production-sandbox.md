...
| `.4.1`   | VM asset contract and validation shape are explicit enough for boot work to start.                      | Asset prepare smoke, manifest validation tests, missing asset fail-closed diagnostics, and source/staging documentation.                                                                              | Final boot-contract correctness if `.4.2.1` proves more assets are needed. |
| `.4.2.1` | Reproducible bootable guest image and boot contract are selected and staged.                            | Pinned image source/build path, selected EFI or kernel/initrd contract, manifest/checksums, helper validation, and local/staging/release acquisition docs.                                            | Successful VM boot or daemon reliability.                                  |
