# Verify human approval and denial through the real reasoning path

Beads: `agent-platform-harness-baseline-p1`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix P1: Does Ask hold execution until a human approves or denies?

Reuse existing UI journey; add real reasoning composition

Pending produces no effect; approval produces exactly one effect; denial produces none after settlement. Correlate approval, tool call, audit and file result.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 5 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
