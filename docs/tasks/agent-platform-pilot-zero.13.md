# Task: Prepare and review the exact single-task orchestration pilot plan

**Beads:** `agent-platform-pilot-zero.13`  
**Parent assessment:** `agent-platform-pilot-zero` (within `agent-platform-multi-agent`)  
**Spec:** `docs/tasks/agent-platform-pilot-zero.13.md`

## Requirements

Select one outstanding permission-coverage task and prepare exact executable material using the reviewed planning skill. Bind task ID, dependency graph, source/branches, roles, allowed actions/files, checks, retries/budget, evidence and completion/delivery boundary. All dynamically linked runtime blockers must be resolved before launch-ready status.

## Allowed boundary

Read Beads/source/skills and readiness evidence. Produce plan/contract and critique evidence in a scoped planning location, with session handoff. No product/runtime edits or live launch.

## Implementation plan

Resolve integration baseline and exact task with repository evidence. Build and schema-validate the material, obtain distinct compliant critique, dispose findings and present concrete scope for owner approval. Record actual approval only when received. If isolated critique or runtime prerequisites cannot be met, expose the blocker instead of substituting an unapproved reviewer or executor.
...
Validate schema, material bindings, references, permissions, task readiness, role separation and negative stale-approval checks. Cross-check the staged assessment protocol. No fabricated execution results, model cost or review verdict.
...
Exact plan is validated, independently reviewed and explicitly owner-approved, with all required prerequisite evidence and clear launch boundary. If approval/readiness is missing, remains open/blocked. The live single-task run is a subsequent authorized action; two-task pilot requires successful first-run review and separate bound material.
