...
5. **Not the segment tip:** push **`task/agent-platform-dx3.3`** to `origin`. **Do not** PR to `feature/agent-platform-mvp` yet. The next task in this segment branches from **`task/agent-platform-dx3.3`**.
...
- [ ] **Branch** **`task/agent-platform-dx3.3`** pushed; next task branches from here (**no** PR to `feature/agent-platform-mvp` until **`task/agent-platform-dx3.4`**)
