# Authorized child tool

Beads issue: `agent-platform-harness-d1`. Status and scheduling authority: Beads.

Parent: `agent-platform-agent-profile-governance`. Proposal label: D1.

## Requirements

Framework child agent using saved configuration; explicit tool/retrieval scopes; depth/concurrency limits; parent/child identity; structured result and parent acceptance.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-r5`, `agent-platform-context-optimisation`, `agent-platform-harness-k1`.

Downstream new records: `agent-platform-harness-d2`.
...
Framework child agent using saved configuration; explicit tool/retrieval scopes; depth/concurrency limits; parent/child identity; structured result and parent acceptance. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
