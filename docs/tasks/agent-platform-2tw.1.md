...
2. Create **`task/agent-platform-2tw.1`** from **`feature/agent-platform-mvp`** (chained Git):  
   `git fetch origin && git checkout feature/agent-platform-mvp && git pull` then `git checkout -b task/agent-platform-2tw.1`.
...
5. **Not the segment tip:** push **`task/agent-platform-2tw.1`** to `origin`. **Do not** PR to `feature/agent-platform-mvp` yet. The next task in this segment branches from **`task/agent-platform-2tw.1`**.
...
| **Chain** | Branch **`task/agent-platform-2tw.1`** from **`feature/agent-platform-mvp`**. |
...
- [ ] **Branch** **`task/agent-platform-2tw.1`** pushed; next task branches from here (**no** PR to `feature/agent-platform-mvp` until **`task/agent-platform-2tw.5`**)
...
- [ ] **Task branch** **`task/agent-platform-2tw.1`** created from **`feature/agent-platform-mvp`** before implementation
