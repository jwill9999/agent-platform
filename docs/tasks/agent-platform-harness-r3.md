# Safe action recovery and human commands

Beads issue: `agent-platform-harness-r3`. Status and scheduling authority: Beads.

Parent: `agent-platform-harness-reliability`. Proposal label: R3.

## Requirements

Stable operation identity; idempotent dispatch where supported; uncertain effect reconciliation; durable approval/clarification; authorized acknowledged cancel; UI gets authoritative waiting/result state.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-r2`.

Downstream new records: `agent-platform-harness-r5`.
...
Stable operation identity; idempotent dispatch where supported; uncertain effect reconciliation; durable approval/clarification; authorized acknowledged cancel; UI gets authoritative waiting/result state. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
...
Reviewer/owner sign-off: pending. Evidence: pending. Effort: 3–6 provisional person-days.
