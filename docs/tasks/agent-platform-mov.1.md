...
2. Follow **Git workflow (mandatory)** below: create **`task/agent-platform-mov.1`** from **`feature/agent-platform-mvp`** before implementation commits.
...
5. Open PR **`task/agent-platform-mov.1` → `feature/agent-platform-mvp`**; merge when reviewed. Do **not** merge this task to `main`.
...
| **Task branch** | Before starting: `git fetch origin && git checkout feature/agent-platform-mvp && git pull` then `git checkout -b task/agent-platform-mov.1`. |
