# Verify plan-driven execution and unfinished steps

Beads: `agent-platform-harness-baseline-x4`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix X4: Does planning govern actual execution and handle unfinished steps?

[Plan generation](../../packages/harness/test/planGenerate.test.ts)

Real planning/parser/graph integration with deterministic external responses: valid multi-step plan, invalid plan and failed step. Capture parsed steps, executed tools, routing and completion state; no unexecuted step represented as complete. Map execution mode first, since not every mode promises plan-driven routing.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 10 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
