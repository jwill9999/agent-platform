...
Beads issue: `agent-platform-harness-r5`. Status and scheduling authority: Beads.

Parent: `agent-platform-harness-reliability`. Proposal label: R5.

## Requirements

Crash before dispatch/after effect/before result save; duplicate resume; permission change; incompatible upgrade; backup/restore including recovery data; browser reload separate from process restart.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-r3`, `agent-platform-harness-r4`.

Downstream new records: `agent-platform-harness-d1`, `agent-platform-harness-b2`.
...
Crash before dispatch/after effect/before result save; duplicate resume; permission change; incompatible upgrade; backup/restore including recovery data; browser reload separate from process restart. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
