# Verify completion criteria revision and terminal outcomes

Beads: `agent-platform-harness-baseline-c3`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix C3: Does completion require the stated criteria to be satisfied?

Real DoD integration; compose with C1 where feasible

Supply unmet then met criteria; confirm corrective feedback, subsequent check and truthful terminal outcome. Cover malformed output and limit exhaustion without substituting evaluator nodes. Record that evaluator evidence is answer/tool messages, not independent file inspection.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 13 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
