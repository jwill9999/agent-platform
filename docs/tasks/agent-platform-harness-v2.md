# Selected-runtime tracing

Beads issue: `agent-platform-harness-v2`. Status and scheduling authority: Beads.

Parent: `agent-platform-llm-observability-export`. Proposal label: V2.

## Requirements

Use supported instrumentation for the F3-selected and R1-implemented dependency set; add narrow application spans at real boundaries; no default SDK4 workaround; one real retry and durations; no duplicate spans; sensitive sentinel absent from exported payload; exporter outage does not break task.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-v1`, `agent-platform-harness-f3`, `agent-platform-harness-r1`.

Downstream new records: `agent-platform-harness-v4`, `agent-platform-harness-v5`.
...
Use supported instrumentation for the F3-selected and R1-implemented dependency set; add narrow application spans at real boundaries; no default SDK4 workaround; one real retry and durations; no duplicate spans; sensitive sentinel absent from exported payload; exporter outage does not break task. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
