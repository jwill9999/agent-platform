# Map and verify migration-affected auxiliary model calls

Beads: `agent-platform-harness-baseline-x7`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix X7: Do sensors and auxiliary model calls retain their contracts if affected by migration?

[Sensor checks](../../packages/harness/test/sensorCheck.test.ts), [sensor traces](../../packages/harness/test/sensorTrace.test.ts), [sensor dashboard](../../apps/api/test/sensorDashboard.integration.test.ts)

Conditional integration inventory: identify direct SDK call sites and exercised parsing, errors, routing and emitted evidence. Test affected callers with external-boundary responses; include connection checks and criterion proposal if the candidate touches them. Keep unaffected areas documented as outside the migration slice.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 18 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
