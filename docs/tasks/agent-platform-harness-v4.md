# Human activity panel

Beads issue: `agent-platform-harness-v4`. Status and scheduling authority: Beads.

Parent: `agent-platform-llm-observability-export`. Proposal label: V4.

## Requirements

Existing UI shows current action/wait, expandable operations and trace link; approval stays in existing authorized path; unknown state honest; works when Phoenix off; pre-run error log path accessible.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-v1`, `agent-platform-harness-v2`, `agent-platform-harness-v3`.

Downstream new records: `agent-platform-harness-v5`, `agent-platform-harness-k4`.
...
Existing UI shows current action/wait, expandable operations and trace link; approval stays in existing authorized path; unknown state honest; works when Phoenix off; pre-run error log path accessible. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
