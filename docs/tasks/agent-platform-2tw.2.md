...
5. **Not the segment tip:** push **`task/agent-platform-2tw.2`** to `origin`. **Do not** PR to `feature/<feature-name>` yet. The next task in this segment branches from **`task/agent-platform-2tw.2`**.
...
- [ ] **Branch** **`task/agent-platform-2tw.2`** pushed; next task branches from here (**no** PR to `feature/<feature-name>` until **`task/agent-platform-2tw.5`**)
