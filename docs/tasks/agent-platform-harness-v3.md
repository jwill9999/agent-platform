# Local Phoenix profile

Beads issue: `agent-platform-harness-v3`. Status and scheduling authority: Beads.

Parent: `agent-platform-llm-observability-export`. Proposal label: V3.

## Requirements

Pinned optional Docker service, separate volume, explicit endpoint, seven-day proposed retention; demonstrate retention configuration, measure disk/CPU/RAM; storage/export failure bounded and visible.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-v1`, `agent-platform-harness-f0`, `agent-platform-harness-f3`.
...
Pinned optional Docker service, separate volume, explicit endpoint, seven-day proposed retention; demonstrate retention configuration, measure disk/CPU/RAM; storage/export failure bounded and visible. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
