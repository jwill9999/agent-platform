# Verify permission scope and hard execution restrictions

Beads: `agent-platform-harness-baseline-p4`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix P4: Are exceptions scoped and hard restrictions preserved?

Backend integration; one UI regression case

Map platform/project/agent/session scope and category precedence from source and persisted settings. Changing one permission must not unintentionally grant unrelated access. Path escape and explicitly denied command stay blocked even after human approval. Use isolated app data because policy may be global.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 8 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
