...
2. Create **`task/agent-platform-2tw.4`** from **`task/agent-platform-2tw.3`** (chained Git):  
...
Complete after work is on **`task/agent-platform-2tw.4`** and tests are green (PR to `feature` only at segment tip).

- [ ] **Task branch** **`task/agent-platform-2tw.4`** created from **`task/agent-platform-2tw.3`** before implementation
