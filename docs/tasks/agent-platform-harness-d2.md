# Child oversight and failure journey

Beads issue: `agent-platform-harness-d2`. Status and scheduling authority: Beads.

Parent: `agent-platform-agent-profile-governance`. Proposal label: D2.

## Requirements

UI child activity, routed approval/clarification, failed child, cancelled parent, aggregate cost; no orphan capacity or unexpected tool authority; integrated cited-source journey.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-d1`, `agent-platform-harness-k4`, `agent-platform-harness-v5`.

Downstream new records: `agent-platform-harness-b2`.
...
UI child activity, routed approval/clarification, failed child, cancelled parent, aggregate cost; no orphan capacity or unexpected tool authority; integrated cited-source journey. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
