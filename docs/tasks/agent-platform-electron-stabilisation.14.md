...
```gherkin
Feature: Common chat attachments

  Background:
    Given the Electron desktop app is running with isolated app data
    And the model provider is stubbed or configured for deterministic chat responses

  Scenario: Attach common files in Personal Chat
    Given I open a new Personal Chat
    When I attach a PNG screenshot and a Markdown file through the chat composer
    Then both files are shown as attachments by user-facing file name
    And the PNG is not rejected as an unsupported text format
    And the Markdown file remains accepted as text context

  Scenario: Attach common files in Project Chat
    Given I open a local Project into Project Chat
    When I attach a JPEG image and a Markdown file through the Project Chat composer
    Then both files are shown as attachments by user-facing file name
    And no host absolute path is shown in the attachment UI
    And sending a message clears the draft attachments after the turn is submitted

  Scenario: Attachment drafts do not leak across chat surfaces
    Given I attach files in Personal Chat without sending them
    When I switch to Workspaces and open a Project Chat
    Then the Project Chat composer does not show the previous attachments
    And returning to a new Personal Chat starts with a clean composer
```
