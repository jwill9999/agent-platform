# Selected modernization and runtime adapter

Beads issue: `agent-platform-harness-r1`. Status and scheduling authority: Beads.

Parent: `agent-platform-harness-reliability`. Proposal label: R1.

## Requirements

Implement only F3-approved package/migration order, including the chosen SDK upgrade or direct framework replacement; one loop owner; preserve provider configuration, streaming, tools, permissions, approvals and Electron behavior; verify native package compatibility; rollback before durable transition. No separate redundant SDK upgrade.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-f3`, `agent-platform-harness-v1`.

Downstream new records: `agent-platform-harness-v2`, `agent-platform-harness-r2`, `agent-platform-harness-r4`.
...
Implement only F3-approved package/migration order, including the chosen SDK upgrade or direct framework replacement; one loop owner; preserve provider configuration, streaming, tools, permissions, approvals and Electron behavior; verify native package compatibility; rollback before durable transition. No separate redundant SDK upgrade. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
...
Reviewer/owner sign-off: pending. Evidence: pending. Effort: 3–7 provisional person-days.
