...
2. Create **`task/agent-platform-mov.2`** from **`task/agent-platform-mov.1`** (chained Git):  
   `git fetch origin && git checkout task/agent-platform-mov.1 && git pull` then `git checkout -b task/agent-platform-mov.2`.
3. Implement with tests per **Tests** section; **unit tests must pass** before sign-off.
4. Ensure `bd dep list agent-platform-mov.2` shows expected upstream Beads issues **closed** where applicable.
5. **Not the segment tip:** push **`task/agent-platform-mov.2`** to `origin`. **Do not** PR to `feature/agent-platform-mvp` yet. The next task in this segment branches from **`task/agent-platform-mov.2`**.
...
| | |
|---|---|
| **Parent for this branch** | **`task/agent-platform-mov.1`** |
| **This task’s branch** | **`task/agent-platform-mov.2`** |
...
| **Chain** | Branch **`task/agent-platform-mov.2`** from **`task/agent-platform-mov.1`**. |
| **Intermediate tasks** | Push **`task/agent-platform-mov.2`**; next task checks out from **`task/agent-platform-mov.2`** (or from remote `origin/task/agent-platform-mov.2`). |
...
- [ ] **Branch** **`task/agent-platform-mov.2`** pushed; next task branches from here (**no** PR to `feature/agent-platform-mvp` until **`task/agent-platform-mov.5`**)
- [ ] This spec file updated if scope or dependencies changed during implementation.

## Sign-off

Complete after work is on **`task/agent-platform-mov.2`** and tests are green (PR to `feature` only at segment tip).

- [ ] **Task branch** **`task/agent-platform-mov.2`** created from **`task/agent-platform-mov.1`** before implementation
