# Source lifecycle and import

Beads issue: `agent-platform-harness-k1`. Status and scheduling authority: Beads.

Parent: `agent-platform-research-tools`. Proposal label: K1.

## Requirements

Parent-owned scope-policy deliverable first: source access, saved-agent permissions and child inheritance agreed with governance; existing memory retained; scoped versioned URL/text/PDF imports using supported loaders; size/access safeguards; import failure visible; refresh/delete state specified.
...
Upstream blocking IDs: `agent-platform-harness-review-gate`, `agent-platform-harness-v1`.

Downstream new records: `agent-platform-harness-k2`, `agent-platform-harness-d1`.
...
Parent-owned scope-policy deliverable first: source access, saved-agent permissions and child inheritance agreed with governance; existing memory retained; scoped versioned URL/text/PDF imports using supported loaders; size/access safeguards; import failure visible; refresh/delete state specified. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
...
Reviewer/owner sign-off: pending. Evidence: pending. Effort: 3–5 provisional person-days.
