# Assess core tool adequacy and agent tool exposure

Beads: `agent-platform-harness-baseline-t2`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix T2: Are missing or overlapping tools obstructing common tasks?

Static inventory plus scenario evidence

Map common task to exposed tool, provider schema, implementation and permission. Identify missing capability, confusing description, redundant calls and distinct reasons for overlap. Do not infer model selection quality from scripted calls. Recommend retain, clarify, restrict exposure or investigate; no tool removal in this plan.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 2 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
