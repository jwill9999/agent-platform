# Verify approval identity replay and argument boundaries

Beads: `agent-platform-harness-baseline-p3`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix P3: Can approvals be fabricated, replayed, transferred or used for changed arguments?

Backend integration tests through actual approval/resume service

Attempt model-supplied approval flags, repeat resume, altered arguments, wrong session/call and prior denial. No unauthorized effect; legitimate scoped human override succeeds once. Record existing binding and any unproven invariant rather than assuming it exists.
