# Verify critic revision acceptance and visible feedback

Beads: `agent-platform-harness-baseline-c1`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix C1: Does the critic accept or request useful revision and display its verdict?

Real evaluator/provider fixture integration and one UI journey

Script revise then accept; inspect evaluator input, critique fed back to reasoning, bounded execution and matching UI feedback. Acceptance with no revision is a separate fast case.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 11 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
