# Whole-task usage policy

Beads issue: `agent-platform-harness-r4`. Status and scheduling authority: Beads.

Parent: `agent-platform-harness-reliability`. Proposal label: R4.

## Requirements

Billable purpose/attempt/child accounting, retries, summaries and fallback; reservation policy; unknown estimates; tests against double-counted aggregation.
...
Billable purpose/attempt/child accounting, retries, summaries and fallback; reservation policy; unknown estimates; tests against double-counted aggregation. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
...
Reviewer/owner sign-off: pending. Evidence: pending. Effort: 2–3 provisional person-days.
