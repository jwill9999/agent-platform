# Beads rebaseline proposal/application

Beads issue: `agent-platform-harness-b1`. Status and scheduling authority: Beads.

Parent: `agent-platform-pre-production-automation`. Proposal label: B1.

## Requirements

Verify full restorable backup before changes; review 32-record audit against fresh state; keep/refine/supersede/defer/evidence-close mapping; resolve ready count and supersedes connector issue; owner reviews proposed mutations before application.
...
Verify full restorable backup before changes; review 32-record audit against fresh state; keep/refine/supersede/defer/evidence-close mapping; resolve ready count and supersedes connector issue; owner reviews proposed mutations before application. Evidence must be linked to exact versions/commit and tests; unknown or unavailable checks are not passes. No old issue closure, removal or supersession without joint review.
