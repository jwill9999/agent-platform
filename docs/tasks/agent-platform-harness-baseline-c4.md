# Investigate missing critic feedback across backend and UI

Beads: `agent-platform-harness-baseline-c4`. Parent: `agent-platform-harness-modernization`.

## Requirements

Matrix C4: Why did critic feedback previously disappear?

UI reproduction with correlated backend evidence

Treat owner's report as unverified. Distinguish node not enabled/reached, evaluator fallback, event not emitted/delivered, parsing/display issue and lost history after reload. Assert only persistence promises actually supported; record missing visibility rather than inventing a diagnosis.
...
Depends on `agent-platform-harness-baseline-map`. Suggested execution position: 14 of 18; order is a scheduling preference, not a claim that all areas technically depend on each other. Shared prerequisite completion does not grant implementation authorization. Review the chosen slice and any production seam first.
