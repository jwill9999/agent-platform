...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Provider configuration    | Configured model, credentials and custom base URL reach the intended provider; Ollama `/v1` behavior remains supported or has an explicit owner-reviewed migration              |
...
| ---------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Provider construction                    | Update SDK provider classes/types and model-call APIs                        | Use supported LangChain model integrations; retire replaced AI SDK provider use | Saved model/base URL/credential configuration and resolution policy  |
...
Additional policy boundary: [tool dispatch](../../../packages/harness/src/nodes/toolDispatch.ts). Framework adapters may invoke this application service; they must not bypass its policy checks. These are source boundaries, not an assertion that each needs rewriting.
...
The currently inspected CLI exposes phase-runtime/coordinator/preflight/status interfaces but no general approved-assessment start entrypoint. The staged pilot-zero documentation records missing start/preapproval composition and runtime-readiness concerns. No exact assessment run/contract, critic execution or ready runtime configuration has been established here. Do not fabricate a run, copy credentials or call a built-in critic and label it managed orchestration.

This source assessment was authored by the parent in supervised mode after the stall was identified. It is **not** the promised critic-reviewed deliverable. Next required step is a supported managed critic launch or explicit owner authorization for a supervised read-only critic fallback, with that deviation recorded. No orchestration repairs are in scope.
