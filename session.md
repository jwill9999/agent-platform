...
- **Date:** 2026-04-19
- **Session:** **Fix MCP Playwright streaming** — Fixed stream hang (tool name sanitization), screenshot rendering (image extraction), Docker workflow, and filename EACCES — branch `task/mcp-image-streaming` (tip `3512bd5`).
...
- **`task/mcp-image-streaming`** — tip `3512bd5` (pushed to origin)
...
- `packages/db` tests fail locally (pre-existing native binding issue — works in Docker)
...
1. **Web UI verification** — Open http://localhost:3001 in Docker and confirm screenshots render inline, submit button recovers, stream completes
2. **Merge PR** — `task/mcp-image-streaming` → `feature/mcp-streaming-fix` → `main`
