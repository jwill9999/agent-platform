...
- **Date:** 2026-05-11
- **Session:** Continued `agent-platform-project-onboarding.8` on `task/agent-platform-project-onboarding.8`: kept slash commands extensible via parser/registry/runner boundaries, registered `/init` as the first built-in command, wired `/v1/chat` to dispatch commands before model execution, and moved Project onboarding draft start into a shared workflow module so slash commands and HTTP routes do not depend on each other. PR #157 is open to `feature/agent-platform-project-onboarding`; SonarCloud's command-sort issue was fixed with explicit `localeCompare`, and the branch is ready to repush after the latest cleanup commit. Focused gates passed: `pnpm --filter @agent-platform/api test -- slashCommands.test.ts`, elevated `pnpm --filter @agent-platform/api test -- sessionChat.integration.test.ts`, elevated `pnpm --filter @agent-platform/api test -- settingsRouter.test.ts`, `pnpm typecheck`, `pnpm format:check`, `pnpm lint`, `pnpm build`, and the first affected-package pre-push pass. Remaining: push the local commits, monitor PR #157 CI/Sonar/Sourcery/reviews, then close `.8` only after all gates are green.
- **Date:** 2026-05-09
