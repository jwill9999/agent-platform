...
- **Today’s outputs:** Updated Project Experience specs and Beads metadata to add `agent-platform-project-experience.9` for Project Chat branch selection and `agent-platform-project-experience.10` for a governed terminal dock using `node-pty` in Electron main, `xterm.js` in the renderer, typed IPC, and Project-root scoping. Re-scoped the Code Workbench epic so it no longer competes with the Project Chat-first direction. Updated the stabilisation closeout spec to record branch selector, terminal dock, previews, activity panel, and external IDE handoff as follow-up work.
- **Validation:** `pnpm docs:lint` and `git diff --check` passed for the documentation changes. Beads shows Project Experience now has ten child tasks, with branch selector depending on Project Chat default surface and terminal dock depending on branch selector.
- **Current state:** Local branch `feature/agent-platform-electron-stabilisation` is ahead by one planning commit: `15737eb feature/agent-platform-electron-stabilisation chore rebaseline project experience plan`. Unrelated local `Makefile` changes and `output/` remain uncommitted and should not be included.
- **Next:** Push the planning commit, then resume `agent-platform-electron-stabilisation.12` closeout/manual QA gate. After stabilisation is accepted, start the Project Experience sequence with the first relevant task.
- **Date:** 2026-05-12
