...
- **Today’s outputs:** Updated Project Experience specs and Beads metadata to add `agent-platform-project-experience.9` for Project Chat branch selection and `agent-platform-project-experience.10` for a governed terminal dock using `node-pty` in Electron main, `xterm.js` in the renderer, typed IPC, and Project-root scoping. Re-scoped the Code Workbench epic so it no longer competes with the Project Chat-first direction. Updated the stabilisation closeout spec and manual QA checklist so Project Chat is the proof path, the built-in file view is secondary/legacy, and branch selector/terminal/preview/activity work is tracked as Project Experience follow-up.
- **Validation:** `pnpm docs:lint` and `git diff --check` passed for the documentation changes. Beads shows Project Experience now has ten child tasks, with branch selector depending on Project Chat default surface and terminal dock depending on branch selector. Bead `agent-platform-electron-stabilisation.12` remains blocked until owner manual QA/sign-off.
- **Current state:** Local branch `feature/agent-platform-electron-stabilisation` has the Project Experience rebaseline pushed and a small docs-only QA checklist update ready to commit/push. Unrelated local `Makefile` changes and `output/` remain uncommitted and should not be included.
- **Next:** Commit/push the QA checklist rebaseline, then complete `agent-platform-electron-stabilisation.12` only after owner reruns or explicitly signs off `docs/qa/electron-project-experience-manual-qa.md`. After stabilisation is accepted, start the Project Experience sequence with the first relevant task.
- **Date:** 2026-05-12
