...
describe('WorkflowCancellationCoordinator', () => {
  it('blocks cancellation until prepared feature delivery operations are recovered', async () => {
    ...
    raw
      .prepare(
        `INSERT INTO delivery_operations
         ...
         VALUES (?, ?, 'run-recovery', 'recovery-feature.9', 'feature.github.pr',
          'workflow_orchestrator', ?, '{}', 'prepared', 'fixture', 0, 0, 0, NULL, 100, 100)`,
      )
      .run(`sha256:${'7'.repeat(64)}`, workspaceId, `sha256:${'7'.repeat(64)}`);
    raw.close();
    const ownerId = 'feature-cancellation-owner';
    const workspaceLeaseEpoch = fixture.store.acquireLease(
      'workspace',
      workspaceId,
      ownerId,
    ...
    const coordinator = WorkflowCancellationCoordinator.createForTest({
      store: fixture.store,
      contract,
      port: OfficialCancellationCleanupPort.createForTest(new IdempotentCleanupPort()),
      clock: () => 1000,
    });
    await expect(
      coordinator.cancel({
        id: 'cancel-feature-prepared',
        runId: 'run-recovery',
    ...
    ).rejects.toThrow('prepared merge must reconcile');
