...
export function validateTransition(
  ...
  if (from === 'approval_waiting') {
    ...
    if (to === 'escalated') {
      if (!context.approval.deadlineElapsed) throw new Error('approval deadline has not elapsed');
      return;
    }
    ...
    if (
      context.approval.authenticatedApproval !== true ||
      context.approval.resumeDeliveryAcknowledged !== true
    ) {
      throw new Error('approval resume requires authenticated approval and acknowledged delivery');
    }
    if (!allowedResumeTargets[context.approval.predecessor]?.includes(to)) {
      throw new Error('approval resume target is not allowed for its predecessor');
    }
    if (context.approval.resumeTarget !== to || context.approval.deadlineElapsed) {
