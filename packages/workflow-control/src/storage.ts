...
export const workflowFeatureDeliveryCriticCapability = Symbol(
  'workflowFeatureDeliveryCriticCapability',
);
export const workflowFeatureDeliveryIntentCapability = Symbol(
...
export class WorkflowStore {
  ...
  createRun(contractId: string, state: WorkflowState = 'approved', id = randomUUID()): RunRecord {
    this.#database.transaction(() => {
      ...
      this.#database
        ...
        .run(id, contractId, state, nowMs, nowMs);
    })();
  ...
  recordFeatureDeliveryRequiredIntent(
    ...
    if (capability !== workflowFeatureDeliveryIntentCapability) {
      throw new Error('feature delivery intent requires the authenticated owner adapter');
    }
    return this.#database
      .transaction(() => {
        const intent = featureDeliveryRequiredIntentSchema.parse(intentInput);
        const contract = this.#contractForRun(intent.runId);
        const executionContractDigest = `sha256:${createHash('sha256')
          .update(JSON.stringify(contract))
          .digest('hex')}`;
        const task = contract.tasks.find((candidate) => candidate.id === intent.taskId);
        if (
          intent.executionContractVersion !== contract.contractVersion ||
          intent.materialDigest !== deriveFeatureDeliveryIntentMaterialDigest(intent) ||
          intent.executionContractDigest !== executionContractDigest ||
          intent.featureId !== contract.featureId ||
          intent.workspaceId !== contract.workspaceId ||
          intent.policyDigest !== contract.policyDigest ||
          intent.repository !== contract.authority.github.repository ||
          intent.featureRef !== contract.authority.github.base ||
          task === undefined
        ) {
          throw new Error('feature delivery intent differs from the exact v1 execution authority');
        }
        const bodyJson = serializeDurableJson(intent);
        const existing = this.#database
          .prepare(
            'SELECT intent_id, body_json FROM feature_delivery_required_intents WHERE run_id = ?',
          )
          .get(intent.runId) as { intent_id: string; body_json: string } | undefined;
        if (existing !== undefined) {
          if (existing.intent_id !== intent.intentId || existing.body_json !== bodyJson) {
            throw new Error('immutable feature delivery intent already differs for this run');
          }
          return intent;
        }
        const runState = this.getRun(intent.runId)?.state;
        if (runState === 'delivery' || runState === 'finalizing' || runState === 'closed') {
          throw new Error('feature delivery intent must be fixed before delivery or finalizing');
        }
        ...
        if (preparedTransition !== undefined) {
          throw new Error('feature delivery intent cannot race a prepared workflow transition');
        }
        this.assertApprovedTaskHead(
          {
            workspaceId: intent.workspaceId,
            runId: intent.runId,
            taskId: intent.taskId,
            headSha: intent.taskHeadSha,
          },
          workflowFeatureDeliveryIntentCapability,
        );
        this.#assertEvidenceReferences(intent.evidence);
        for (const reference of intent.evidence) {
          if (
            !this.hasTaskEvidenceBinding({
              ...reference,
              workspaceId: intent.workspaceId,
              runId: intent.runId,
              taskId: intent.taskId,
              contractVersion: intent.executionContractVersion,
              policyDigest: intent.policyDigest,
              allowedProducerRoles: ['plan_critic', 'workflow_orchestrator'],
            })
          ) {
            throw new Error('feature delivery intent evidence is not exact and run/task-bound');
          }
        }
        const identityCollision = this.#database
          .prepare('SELECT 1 FROM feature_delivery_required_intents WHERE intent_id = ?')
          .get(intent.intentId);
        if (identityCollision !== undefined) {
          throw new Error('feature delivery intent identity is already bound to another run');
        }
        this.#database
          .prepare(
            `INSERT INTO feature_delivery_required_intents
         (run_id, intent_id, body_json, created_at_ms) VALUES (?, ?, ?, ?)`,
          )
          .run(intent.runId, intent.intentId, bodyJson, intent.decidedAtMs);
        return intent;
      })
      .immediate();
  }

  getFeatureDeliveryRequiredIntent(runId: string): FeatureDeliveryRequiredIntent | undefined {
    const row = this.#database
      .prepare('SELECT body_json FROM feature_delivery_required_intents WHERE run_id = ?')
      .get(runId) as { body_json: string } | undefined;
    return row === undefined
      ? undefined
      : featureDeliveryRequiredIntentSchema.parse(JSON.parse(row.body_json) as unknown);
  }

  #assertFeaturePostOriginEvidence(
    ...
    const origin = this.getDeliveryOperation(contract.origin.integrationMergeOperationId);
    const originRequest = origin?.request as Record<string, unknown> | undefined;
    const originResult = origin?.result as Record<string, unknown> | undefined;
    const attestationDigest =
      originResult === undefined
        ? undefined
        : `sha256:${createHash('sha256').update(JSON.stringify(originResult)).digest('hex')}`;
    if (
      origin?.status !== 'committed' ||
      origin.runId !== runId ||
      origin.taskId !== taskId ||
      origin.kind !== 'github.merge' ||
      originRequest?.headSha !== contract.origin.taskHeadSha ||
      originRequest.base !== contract.origin.featureRef ||
      originResult?.headSha !== contract.origin.taskHeadSha ||
      originResult.mergeSha !== contract.origin.integratedHeadSha ||
      attestationDigest !== contract.origin.integrationMergeAttestationDigest
    ) {
      throw new Error('feature delivery evidence lacks the exact committed origin merge');
    }
    const placeholders = allowedProducerRoles.map(() => '?').join(', ');
    for (const reference of evidence) {
      const binding = this.#database
        .prepare(
          `SELECT 1 FROM evidence_bindings AS binding
           JOIN evidence ON evidence.digest = binding.digest
           WHERE binding.digest = ? AND evidence.media_type = ? AND evidence.size_bytes = ?
             AND binding.kind = ? AND binding.workspace_id = ? AND binding.run_id = ?
             AND binding.task_id = ? AND binding.contract_version = ? AND binding.policy_digest = ?
             AND binding.producer_role IN (${placeholders}) AND binding.head_sha = ?
             AND binding.operation_id = ? AND binding.created_at_ms >= ? LIMIT 1`,
        )
        .get(
          reference.digest,
          reference.mediaType,
          ...
          runId,
          taskId,
          contract.origin.executionContractVersion,
          contract.policyDigest,
          ...allowedProducerRoles,
          contract.origin.integratedHeadSha,
          contract.origin.integrationMergeOperationId,
          origin.updatedAtMs,
        );
      if (binding === undefined) {
        throw new Error(
          'feature delivery evidence is not bound to the integrated head and exact committed origin',
        );
      }
    }
  }

  recordFeatureDeliveryCriticReview(
    reviewInput: unknown,
    contractInput: unknown,
    capability?: symbol,
  ): FeatureDeliveryCriticReview {
    if (capability !== workflowFeatureDeliveryCriticCapability) {
      throw new Error('feature delivery critic review requires the authenticated critic adapter');
    }
    const review = featureDeliveryCriticReviewSchema.parse(reviewInput);
    const featureContract = featureDeliveryContractSchema.parse(contractInput);
    const executionContract = this.#contractForRun(review.runId);
    const executionDigest = `sha256:${createHash('sha256')
      .update(JSON.stringify(executionContract))
      .digest('hex')}`;
    const intent = this.getFeatureDeliveryRequiredIntent(review.runId);
    if (
      review.taskId !== featureContract.origin.taskId ||
      review.executionContractDigest !== executionDigest ||
      review.executionContractDigest !== featureContract.executionContractDigest ||
      review.featureContractVersion !== featureContract.contractVersion ||
      review.featureContractDigest !== deriveFeatureDeliveryContractDigest(featureContract) ||
      review.materialDigest !== deriveFeatureDeliveryMaterialDigest(featureContract) ||
      review.policyDigest !== featureContract.policyDigest
    ) {
      throw new Error('feature delivery critic review is stale or bound to different material');
    }
    if (intent === undefined) {
      throw new Error('feature delivery critic review requires explicit pre-merge intent');
    }
    if (
      intent.taskId !== featureContract.origin.taskId ||
      intent.taskHeadSha !== featureContract.origin.taskHeadSha ||
      intent.featureRef !== featureContract.origin.featureRef ||
      intent.repository !== featureContract.origin.repository ||
      intent.executionContractDigest !== featureContract.executionContractDigest ||
      intent.protectionDigest !== featureContract.authority.protectionDigest ||
      JSON.stringify(
        [...intent.requiredChecks].sort((left, right) => left.localeCompare(right)),
      ) !==
        JSON.stringify(
          [...featureContract.authority.requiredChecks].sort((left, right) =>
            left.localeCompare(right),
          ),
        )
    ) {
      throw new Error('feature delivery critic contract differs from the pre-merge intent');
    }
    const reviewEvidence = [
      ...review.evidence,
      ...review.findings.flatMap((finding) => finding.evidence),
    ];
    this.#assertEvidenceReferences(reviewEvidence);
    this.#assertFeaturePostOriginEvidence(
      featureContract,
      review.runId,
      review.taskId,
      reviewEvidence,
    ...
    const bodyJson = serializeDurableJson(review);
    this.#database
      .prepare(
        `INSERT OR IGNORE INTO feature_delivery_critic_reviews
         (id, run_id, task_id, feature_contract_digest, verdict, body_json, reviewed_at_ms)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        review.reviewId,
        review.runId,
        review.taskId,
        review.featureContractDigest,
        review.verdict,
        bodyJson,
        review.reviewedAtMs,
      );
    const stored = this.#database
      .prepare('SELECT body_json FROM feature_delivery_critic_reviews WHERE id = ?')
      .get(review.reviewId) as { body_json: string };
    if (stored.body_json !== bodyJson) {
      throw new Error('immutable feature delivery critic review identity changed');
    }
    return review;
  }

  recordFeatureDeliveryCriticReviewForTest(
    reviewInput: unknown,
    contractInput: unknown,
  ): FeatureDeliveryCriticReview {
    if (process.env.NODE_ENV !== 'test') throw new Error('feature critic test adapter unavailable');
    return this.recordFeatureDeliveryCriticReview(
      reviewInput,
      contractInput,
      workflowFeatureDeliveryCriticCapability,
    );
  ...
  createFeatureDeliveryApproval(
    approvalInput: unknown,
    contractInput: unknown,
    capability?: symbol,
  ): FeatureDeliveryApproval {
    if (capability !== workflowFeatureDeliveryApprovalCapability) {
    ...
    const executionContractDigest = `sha256:${createHash('sha256')
      ...
      .digest('hex')}`;
    if (
      approval.featureContractVersion !== contract.contractVersion ||
      approval.featureContractDigest !== deriveFeatureDeliveryContractDigest(contract) ||
      approval.materialDigest !== deriveFeatureDeliveryMaterialDigest(contract) ||
      approval.policyDigest !== contract.policyDigest ||
      approval.taskId !== contract.origin.taskId ||
      contract.featureId !== executionContract.featureId ||
      contract.workspaceId !== executionContract.workspaceId ||
      contract.policyDigest !== executionContract.policyDigest ||
      contract.origin.executionContractVersion !== executionContract.contractVersion ||
      contract.origin.repository !== executionContract.authority.github.repository ||
      contract.executionContractDigest !== executionContractDigest
    ) {
      throw new Error('feature delivery approval is stale or bound to different material');
    }
    if (intent === undefined) {
      throw new Error('feature delivery approval requires explicit pre-merge intent');
    }
    ...
    return this.#database.transaction(() => {
      ...
      if (replay !== undefined) {
        if (
          replay.runId !== approval.runId ||
          replay.reviewId !== approval.reviewId ||
          replay.taskId !== approval.taskId ||
          replay.approverId !== approval.approverId ||
          replay.approverRole !== approval.approverRole ||
          replay.approvalVersion !== approval.approvalVersion ||
          replay.featureContractVersion !== approval.featureContractVersion ||
          replay.featureContractDigest !== approval.featureContractDigest ||
          replay.materialDigest !== approval.materialDigest ||
          replay.policyDigest !== approval.policyDigest ||
          replay.approvedAtMs !== approval.approvedAtMs ||
          JSON.stringify(replay.evidence) !== JSON.stringify(approval.evidence)
        ) {
          throw new Error('feature delivery approval replay changes immutable identity');
        }
        return replay;
      }
      this.#assertEvidenceReferences(approval.evidence);
      const latestReview = this.#database
        .prepare(
          `SELECT body_json FROM feature_delivery_critic_reviews
           WHERE run_id = ? ORDER BY reviewed_at_ms DESC, id DESC LIMIT 1`,
        )
        .get(approval.runId) as { body_json: string } | undefined;
      const parsedReview =
        latestReview === undefined
          ? undefined
          : featureDeliveryCriticReviewSchema.parse(JSON.parse(latestReview.body_json) as unknown);
      if (
        parsedReview?.verdict !== 'approved' ||
        parsedReview.reviewId !== approval.reviewId ||
        parsedReview.findings.length !== 0 ||
        parsedReview.featureContractDigest !== approval.featureContractDigest ||
        parsedReview.materialDigest !== approval.materialDigest
      ) {
        throw new Error('feature delivery approval lacks the exact latest approved critic review');
      }
      const reviewedEvidence = new Set(parsedReview.evidence.map((reference) => reference.digest));
      for (const reference of approval.evidence) {
        if (!reviewedEvidence.has(reference.digest)) {
          throw new Error(
            'feature delivery approval evidence is not critic-reviewed and run/task-bound',
          );
        }
      }
      this.#assertFeaturePostOriginEvidence(
      ...
      this.#database
        .prepare(
          `UPDATE feature_delivery_approvals SET status = 'invalidated', invalidated_at_ms = ?,
           invalidation_reason = 'superseded'
           WHERE run_id = ? AND status = 'active'`,
        )
        .run(approval.approvedAtMs, approval.runId);
      this.#database
        .prepare(
          `INSERT INTO feature_delivery_approvals
           (id, run_id, review_id, task_id, approver_id, approver_role, approval_version, feature_contract_version,
            feature_contract_digest, material_digest, policy_digest, status, approved_at_ms,
            invalidated_at_ms, invalidation_reason, evidence_json)
        ...
        .run(
          ...
          approval.featureContractVersion,
          approval.featureContractDigest,
          approval.materialDigest,
          approval.policyDigest,
          approval.approvedAtMs,
          JSON.stringify(approval.evidence),
        );
      return this.getFeatureDeliveryApproval(approval.approvalId)!;
    })();
  }

  createFeatureDeliveryApprovalForTest(
    approvalInput: unknown,
    contractInput: unknown,
  ): FeatureDeliveryApproval {
    if (process.env.NODE_ENV !== 'test') {
      throw new Error('feature delivery approval test adapter is unavailable outside tests');
    }
    const approval = {
      reviewId: 'test-approved-feature-critic-review',
      taskId: featureDeliveryContractSchema.parse(contractInput).origin.taskId,
      evidence: [
        {
    ...
    const contract = featureDeliveryContractSchema.parse(contractInput);
    const parsed = featureDeliveryApprovalSchema.parse(approval);
    return this.#database.transaction(() => {
      const executionContract = this.#contractForRun(parsed.runId);
      const executionDigest = `sha256:${createHash('sha256')
        .update(JSON.stringify(executionContract))
        .digest('hex')}`;
      if (
        parsed.featureContractDigest !== deriveFeatureDeliveryContractDigest(contract) ||
        parsed.materialDigest !== deriveFeatureDeliveryMaterialDigest(contract) ||
        contract.executionContractDigest !== executionDigest ||
        parsed.policyDigest !== contract.policyDigest
      ) {
        throw new Error('feature delivery approval is stale or bound to different material');
      }
      this.#database
        .prepare(
          `INSERT INTO feature_delivery_approvals
           (id, run_id, review_id, task_id, approver_id, approver_role, approval_version,
            feature_contract_version, feature_contract_digest, material_digest, policy_digest,
            status, approved_at_ms, invalidated_at_ms, invalidation_reason, evidence_json)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, NULL, NULL, ?)`,
        )
        .run(
          parsed.approvalId,
          ...
          parsed.approverId,
          parsed.approverRole,
          parsed.approvalVersion,
          parsed.featureContractVersion,
          parsed.featureContractDigest,
          parsed.materialDigest,
          parsed.policyDigest,
          parsed.approvedAtMs,
          JSON.stringify(parsed.evidence),
        );
      return this.getFeatureDeliveryApproval(parsed.approvalId)!;
    })();
  }

  invalidateFeatureDeliveryApproval(
    approvalId: string,
    reason: string,
    invalidatedAtMs = Date.now(),
  ): FeatureDeliveryApproval {
    if (reason.trim() === '') throw new Error('feature delivery invalidation reason is required');
    return this.#database
      .transaction(() => {
        const approval = this.getFeatureDeliveryApproval(approvalId);
        if (approval === undefined || approval.status !== 'active') {
          throw new Error('active feature delivery approval not found');
        }
        const prepared = this.#database
          .prepare(
            `SELECT 1 FROM delivery_operations
           WHERE run_id = ? AND status = 'prepared' AND kind LIKE 'feature.github.%' LIMIT 1`,
          )
          .get(approval.runId);
        if (prepared !== undefined) {
          throw new Error(
            'feature delivery approval cannot be invalidated while an effect is prepared',
          );
        }
        ...
        if (changed.changes !== 1) throw new Error('active feature delivery approval not found');
        return this.getFeatureDeliveryApproval(approvalId)!;
      })
      .immediate();
  }

  getFeatureDeliveryApproval(approvalId: string): FeatureDeliveryApproval | undefined {
    const row = this.#database
      .prepare('SELECT * FROM feature_delivery_approvals WHERE id = ?')
    ...
    return row === undefined
      ...
      : featureDeliveryApprovalSchema.parse({
          ...
          featureContractVersion: row.feature_contract_version,
          featureContractDigest: row.feature_contract_digest,
          ...
          evidence: JSON.parse(row.evidence_json as string) as unknown,
        });
  }

  recordFeatureDeliveryContract(
    input: {
      runId: string;
      workspaceId: string;
    ...
    this.#database.transaction(() => {
      ...
      if (
        ...
        executionContract.contractVersion !== input.executionContractVersion ||
        executionContract.policyDigest !== input.policyDigest
      ) {
        throw new Error('feature delivery contract has a stale execution origin');
      }
      if (this.getRun(input.runId)?.state !== 'finalizing') {
        throw new Error('feature delivery contract may only be fixed during finalizing');
      }
      ...
      const bodyJson = serializeDurableJson(input.contract);
      const actualDigest = `sha256:${createHash('sha256').update(bodyJson).digest('hex')}`;
      if (actualDigest !== input.contractDigest) {
        throw new Error('feature delivery contract digest differs from its canonical content');
      }
      this.#assertActiveFeatureDeliveryApproval(
        input.runId,
        input.featureContractVersion,
      ...
      this.#database
        .prepare(
          `INSERT OR IGNORE INTO feature_delivery_contracts
      ...
      const stored = this.#database
        .prepare('SELECT * FROM feature_delivery_contracts WHERE run_id = ?')
        .get(input.runId) as
        | {
            feature_contract_version: number;
        ...
        | undefined;
      if (
        stored?.feature_contract_version !== input.featureContractVersion ||
        stored.contract_digest !== input.contractDigest ||
        stored.execution_contract_digest !== input.executionContractDigest ||
        stored.body_json !== bodyJson
      ) {
        throw new Error('immutable feature delivery contract already differs for this run');
      }
    })();
  }

  #assertActiveFeatureDeliveryApproval(
    runId: string,
    featureContractVersion: number,
    contractDigest: string,
    executionContractDigest: string,
    ...
    const approval = this.#database
      .prepare(
        `SELECT review_id FROM feature_delivery_approvals
         WHERE run_id = ? AND feature_contract_version = ? AND feature_contract_digest = ?
           AND material_digest = ? AND policy_digest = ? AND status = 'active'
      ...
      .get(
        ...
        contractDigest,
        deriveFeatureDeliveryMaterialDigest(contract),
        policyDigest,
      );
    const parsed = featureDeliveryContractSchema.parse(contract);
    const intent = this.#database
      .prepare('SELECT body_json FROM feature_delivery_required_intents WHERE run_id = ?')
      .get(runId) as { body_json: string } | undefined;
    if (intent !== undefined) {
      const body = featureDeliveryRequiredIntentSchema.parse(
        JSON.parse(intent.body_json) as unknown,
      );
      if (
        body.featureId !== parsed.featureId ||
        body.workspaceId !== parsed.workspaceId ||
        body.repository !== parsed.authority.repository ||
        body.featureRef !== parsed.authority.headRef ||
        body.destination !== parsed.authority.base ||
        body.executionContractDigest !== parsed.executionContractDigest ||
        body.policyDigest !== parsed.policyDigest ||
        body.taskId !== parsed.origin.taskId ||
        body.taskRef !== parsed.origin.taskRef ||
        body.taskHeadSha !== parsed.origin.taskHeadSha ||
        JSON.stringify(
          [...body.requiredChecks].sort((left, right) => left.localeCompare(right)),
        ) !==
          JSON.stringify(
            [...parsed.authority.requiredChecks].sort((left, right) => left.localeCompare(right)),
          ) ||
        body.protectionDigest !== parsed.authority.protectionDigest ||
        body.mergeMethod !== parsed.authority.mergeMethod ||
        body.adminBypass !== parsed.authority.adminBypass
      ) {
        throw new Error('feature delivery contract differs from the durable required intent');
      }
    }
    const testApproval =
      process.env.NODE_ENV === 'test' &&
      (approval as { review_id?: string } | undefined)?.review_id ===
        'test-approved-feature-critic-review';
    const exactReview =
      approval === undefined || testApproval
        ? testApproval
        : this.#hasExactApprovedFeatureCriticReview(
            runId,
            (approval as { review_id: string }).review_id,
            contractDigest,
            deriveFeatureDeliveryMaterialDigest(parsed),
          );
    if (
      approval === undefined ||
      exactReview !== true ||
      parsed.executionContractDigest !== executionContractDigest ||
      deriveFeatureDeliveryContractDigest(parsed) !== contractDigest
    ) {
      throw new Error('active feature delivery approval is unavailable or stale');
    }
  }

  assertRunUsesFeatureDeliveryContract(
    runId: string,
    contractDigest: string,
    contract: unknown,
  ): void {
    const bodyJson = serializeDurableJson(contract);
    const stored = this.#database
      .prepare(`SELECT contract_digest, body_json FROM feature_delivery_contracts WHERE run_id = ?`)
      .get(runId) as { contract_digest: string; body_json: string } | undefined;
    if (stored?.contract_digest !== contractDigest || stored.body_json !== bodyJson) {
      throw new Error('workflow run does not use the immutable feature delivery contract');
    }
  }

  getApprovedFeatureDeliveryContract(runId: string): unknown | undefined {
    const anyAuthority = this.#database
      .prepare(
        `SELECT
           EXISTS(SELECT 1 FROM feature_delivery_contracts WHERE run_id = ?) AS has_contract,
           EXISTS(SELECT 1 FROM feature_delivery_approvals WHERE run_id = ?) AS has_approval,
           EXISTS(SELECT 1 FROM feature_delivery_required_intents WHERE run_id = ?) AS has_intent`,
      )
    ...
    if (
      anyAuthority.has_contract === 0 &&
      anyAuthority.has_approval === 0 &&
      anyAuthority.has_intent === 0
    )
      return undefined;
    const row = this.#database
      .prepare(
        `SELECT contracts.body_json
         FROM feature_delivery_contracts AS contracts
         ...
         WHERE contracts.run_id = ? AND approvals.status = 'active'
           AND approvals.approver_role = 'human_approver'
         LIMIT 1`,
      )
      .get(runId) as { body_json: string } | undefined;
    if (row === undefined) {
      throw new Error('durable feature delivery authority lacks an active exact approval');
    }
    const contract = featureDeliveryContractSchema.parse(JSON.parse(row.body_json) as unknown);
    this.#assertActiveFeatureDeliveryApproval(
      runId,
      contract.contractVersion,
      deriveFeatureDeliveryContractDigest(contract),
  ...
  assertApprovedTaskHead(
    ...
    if (
      ...
      capability !== workflowFeatureDeliveryIntentCapability
    ) {
  ...
  assertApprovedEvaluationHead(
    ...
    if (featureContract !== undefined) {
      ...
      if (
        ...
        parsed.origin.taskId !== input.taskId ||
        parsed.origin.integratedHeadSha !== input.headSha
      ) {
        throw new Error('evaluation head is not the exact approved integrated feature head');
      }
      return;
    }
  ...
  getCommittedFeatureDeliveryAttestation(
    runId: string,
    operationId?: string,
    ...
    const row = this.#database
      .prepare(
        `SELECT * FROM delivery_operations
         WHERE run_id = ? AND kind = 'feature.github.merge' AND status = 'committed'
           AND (? IS NULL OR id = ?)
         ORDER BY updated_at_ms DESC, id DESC LIMIT 1`,
      )
      .get(runId, operationId ?? null, operationId ?? null) as DeliveryOperationRow | undefined;
    return row === undefined ? undefined : deliveryOperationFromRow(row);
  }
  ...
  listPreparedDeliveryOperations(runId: string): DeliveryOperationRecord[] {
    return (
      this.#database
        .prepare(
          `SELECT * FROM delivery_operations
           WHERE run_id = ? AND status = 'prepared' AND kind NOT LIKE 'feature.github.%'
  ...
  listPreparedFeatureDeliveryOperations(runId: string): DeliveryOperationRecord[] {
    return (
      this.#database
        .prepare(
          `SELECT * FROM delivery_operations
           WHERE run_id = ? AND status = 'prepared' AND kind LIKE 'feature.github.%'
           ORDER BY created_at_ms, id`,
        )
        .all(runId) as DeliveryOperationRow[]
    ).map(deliveryOperationFromRow);
  ...
  listRunsWithPreparedFeatureDeliveryOperations(workspaceId: string): string[] {
    return (
      this.#database
        .prepare(
          `SELECT DISTINCT operations.run_id
           FROM delivery_operations AS operations
           JOIN runs ON runs.id = operations.run_id
           JOIN contracts ON contracts.id = runs.contract_id
           WHERE operations.status = 'prepared' AND operations.kind LIKE 'feature.github.%'
             AND json_extract(contracts.body_json, '$.workspaceId') = ?
           ORDER BY operations.run_id`,
        )
        .all(workspaceId) as Array<{ run_id: string }>
    ).map((row) => row.run_id);
  }
  ...
  adoptPreparedDeliveryOperation(
    ...
    return this.#database.transaction(() => {
      ...
      this.#assertActiveFeatureAuthorityForOperation(operation);
      this.#assertDeliveryRunState(operation.runId, operation.kind);
      this.#assertResourceLease(
        'workspace',
  ...
  #assertActiveFeatureAuthorityForOperation(operation: DeliveryOperationRecord): void {
    if (!operation.kind.startsWith('feature.github.')) return;
    const request = operation.request as Record<string, unknown> | null | undefined;
    if (
      request === undefined ||
      request === null ||
      typeof request.featureContractVersion !== 'number' ||
      typeof request.featureContractDigest !== 'string' ||
      typeof request.policyDigest !== 'string'
    ) {
      if (this.getApprovedFeatureDeliveryContract(operation.runId) === undefined) {
        throw new Error('feature delivery operation lacks approved contract identity');
      }
      return;
    }
    const authority = this.#database
      .prepare(
        `SELECT approvals.review_id FROM feature_delivery_contracts AS contracts
         JOIN feature_delivery_approvals AS approvals
           ON approvals.run_id = contracts.run_id
          AND approvals.feature_contract_version = contracts.feature_contract_version
          AND approvals.feature_contract_digest = contracts.contract_digest
      ...
      .get(
        operation.runId,
        request.featureContractVersion,
        request.featureContractDigest,
        request.policyDigest,
      );
    const testAuthority =
      process.env.NODE_ENV === 'test' &&
      (authority as { review_id?: string } | undefined)?.review_id ===
        'test-approved-feature-critic-review';
    const contract = this.#database
      .prepare('SELECT body_json FROM feature_delivery_contracts WHERE run_id = ?')
      .get(operation.runId) as { body_json: string } | undefined;
    const parsedContract =
      contract === undefined
        ? undefined
        : featureDeliveryContractSchema.parse(JSON.parse(contract.body_json) as unknown);
    const exactReview =
      authority === undefined || parsedContract === undefined || testAuthority
        ? testAuthority
        : this.#hasExactApprovedFeatureCriticReview(
            operation.runId,
            (authority as { review_id: string }).review_id,
            deriveFeatureDeliveryContractDigest(parsedContract),
            deriveFeatureDeliveryMaterialDigest(parsedContract),
          );
    if (authority === undefined || parsedContract === undefined || exactReview !== true) {
      throw new Error('feature delivery operation lacks active approved authority');
    }
  }

  #hasExactApprovedFeatureCriticReview(
    runId: string,
    reviewId: string,
    featureContractDigest: string,
    ...
    const review = featureDeliveryCriticReviewSchema.parse(JSON.parse(row.body_json) as unknown);
    return (
      review.reviewId === reviewId &&
      review.verdict === 'approved' &&
      review.findings.length === 0 &&
      review.featureContractDigest === featureContractDigest &&
      review.materialDigest === materialDigest
    );
  }
  ...
  #assertDeliveryLineage(operation: DeliveryOperationRecord): void {
    ...
    if (operation.kind.startsWith('feature.github.')) {
      this.#assertFeatureDeliveryLineage(operation, request);
      return;
    }
  ...
  #assertFeatureDeliveryLineage(
    operation: DeliveryOperationRecord,
    request: Record<string, unknown> | undefined,
  ): void {
    const originOperationId = request?.originMergeOperationId;
    const originAttestationDigest = request?.originMergeAttestationDigest;
    const originPullRequestNumber = request?.originPullRequestNumber;
    const taskHeadSha = request?.originTaskHeadSha;
    const featureHeadSha = request?.headSha;
    const featureRef = request?.headRef;
    if (
      typeof originOperationId !== 'string' ||
      typeof originAttestationDigest !== 'string' ||
      typeof originPullRequestNumber !== 'number' ||
      typeof taskHeadSha !== 'string' ||
      typeof featureHeadSha !== 'string' ||
      typeof featureRef !== 'string'
    ) {
      throw new Error('feature delivery request has invalid integration lineage');
    }
    const origin = this.getDeliveryOperation(originOperationId);
    const originRequest = origin?.request as Record<string, unknown> | undefined;
    const originResult = origin?.result as Record<string, unknown> | null | undefined;
    const serializedOriginResult = JSON.stringify(originResult);
    const observedDigest =
      serializedOriginResult === undefined
        ? undefined
        : `sha256:${createHash('sha256').update(serializedOriginResult).digest('hex')}`;
    if (
      origin === undefined ||
      origin.status !== 'committed' ||
      origin.kind !== 'github.merge' ||
      origin.runId !== operation.runId ||
      origin.workspaceId !== operation.workspaceId ||
      origin.taskId !== operation.taskId ||
      originRequest?.headSha !== taskHeadSha ||
      originRequest.pullRequestNumber !== originPullRequestNumber ||
      originRequest.base !== featureRef ||
      originRequest.mergeMethod !== 'squash' ||
      originResult?.headSha !== taskHeadSha ||
      originResult.pullRequestNumber !== originPullRequestNumber ||
      originResult.base !== featureRef ||
      originResult.mergeMethod !== 'squash' ||
      originResult.mergeSha !== featureHeadSha ||
      observedDigest !== originAttestationDigest
    ) {
      throw new Error('feature delivery head lacks the exact committed integration attestation');
    }
  }
      ...
      this.#database
        .prepare(
          `INSERT OR IGNORE INTO evidence_bindings
           (digest, workspace_id, run_id, task_id, transition_id, operation_id, contract_version, policy_digest,
  ...
  recordFeatureFinalization(
    ...
    return this.#database.transaction(() => {
      ...
      const durableFeatureContract = this.getApprovedFeatureDeliveryContract(input.runId);
      const featureContract =
        durableFeatureContract === undefined
          ? undefined
          : featureDeliveryContractSchema.parse(durableFeatureContract);
      const mergedHeadSha = featureContract?.origin.integratedHeadSha ?? mergeRequest?.headSha;
      ...
      if (input.featureDelivery !== undefined) {
        const delivery = this.getCommittedFeatureDeliveryAttestation(
          input.runId,
          input.featureDelivery.operationId,
        );
        const deliveryRequest = delivery?.request as Record<string, unknown> | undefined;
        const deliveryResult = delivery?.result as Record<string, unknown> | undefined;
        if (
          deliveryRequest?.featureContractDigest !== input.featureDelivery.contractDigest ||
          deliveryRequest.headSha !== input.featureDelivery.integratedHeadSha ||
          deliveryRequest.base !== input.featureDelivery.destination ||
          deliveryRequest.mergeMethod !== 'squash' ||
          deliveryRequest.adminBypass !== false ||
          deliveryResult?.headSha !== input.featureDelivery.integratedHeadSha ||
          deliveryResult.base !== input.featureDelivery.destination ||
          deliveryResult.mergeMethod !== 'squash' ||
          deliveryResult.mergeSha !== input.featureDelivery.mergeSha ||
          deliveryResult.eventIdentity !== input.featureDelivery.eventIdentity ||
          report.destination !== input.featureDelivery.destination ||
          report.mergeSha !== input.featureDelivery.mergeSha ||
          report.mergeEventIdentity !== input.featureDelivery.eventIdentity
        ) {
          throw new Error('final report lacks the exact committed feature delivery attestation');
        }
      }
  ...
  #migrate(): void {
    this.#database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS feature_delivery_contracts (
        run_id TEXT PRIMARY KEY REFERENCES runs(id),
        feature_contract_version INTEGER NOT NULL,
        contract_digest TEXT NOT NULL UNIQUE,
        execution_contract_digest TEXT NOT NULL,
        body_json TEXT NOT NULL,
        created_at_ms INTEGER NOT NULL
      );
      CREATE TABLE IF NOT EXISTS feature_delivery_required_intents (
        run_id TEXT PRIMARY KEY REFERENCES runs(id),
        intent_id TEXT NOT NULL UNIQUE,
        body_json TEXT NOT NULL,
        created_at_ms INTEGER NOT NULL
      );
      CREATE TABLE IF NOT EXISTS feature_delivery_critic_reviews (
        id TEXT PRIMARY KEY,
        run_id TEXT NOT NULL REFERENCES runs(id),
        task_id TEXT NOT NULL,
        feature_contract_digest TEXT NOT NULL,
        verdict TEXT NOT NULL CHECK (verdict IN ('approved', 'correction_required')),
        body_json TEXT NOT NULL,
        reviewed_at_ms INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS feature_delivery_critic_reviews_latest
        ON feature_delivery_critic_reviews(run_id, reviewed_at_ms DESC, id DESC);
      CREATE TABLE IF NOT EXISTS feature_delivery_approvals (
        id TEXT PRIMARY KEY,
        run_id TEXT NOT NULL REFERENCES runs(id),
        review_id TEXT NOT NULL,
        task_id TEXT NOT NULL,
        approver_id TEXT NOT NULL,
        approver_role TEXT NOT NULL CHECK (approver_role = 'human_approver'),
      ...
      CREATE UNIQUE INDEX IF NOT EXISTS feature_delivery_approvals_one_active
        ON feature_delivery_approvals(run_id) WHERE status = 'active';
    ...
    const featureIntentColumns = this.#database
      ...
      .all() as Array<{ name: string }>;
    if (!featureIntentColumns.some((column) => column.name === 'intent_id')) {
      this.#database.exec(
        `ALTER TABLE feature_delivery_required_intents ADD COLUMN intent_id TEXT;
         UPDATE feature_delivery_required_intents SET intent_id = 'legacy:' || run_id
           WHERE intent_id IS NULL;
         CREATE UNIQUE INDEX IF NOT EXISTS feature_delivery_required_intents_identity
           ON feature_delivery_required_intents(intent_id);`,
      );
    }
    ...
    const evidenceBindingColumns = this.#database
      .prepare('PRAGMA table_info(evidence_bindings)')
      .all() as Array<{ name: string }>;
    if (!evidenceBindingColumns.some((column) => column.name === 'operation_id')) {
      this.#database.exec(
        "ALTER TABLE evidence_bindings ADD COLUMN operation_id TEXT NOT NULL DEFAULT ''",
      );
    }
