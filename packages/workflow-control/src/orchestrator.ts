...
import {
  isRegisteredFeatureDeliveryRecoveryDriver,
  type FeatureDeliveryRecoveryDriver,
...
export class WorkflowOrchestrator {
  ...
  readonly #featureDeliveryRecovery: FeatureDeliveryRecoveryDriver | undefined;
  ...
  constructor(input: {
    ...
    ownerId: string;
    clock?: () => number;
    featureDeliveryRecovery?: FeatureDeliveryRecoveryDriver;
  }) {
    ...
    if (
      input.featureDeliveryRecovery !== undefined &&
      !isRegisteredFeatureDeliveryRecoveryDriver(
        input.featureDeliveryRecovery,
        input.store,
        input.contract,
      )
    ) {
      throw new Error('orchestrator requires a registered exact feature recovery driver');
    }
    ...
    this.#featureDeliveryRecovery = input.featureDeliveryRecovery;
  }
  ...
  async reconcileAfterRestart(input: {
    ...
    for (const runId of this.#store.listRunsWithPreparedFeatureDeliveryOperations(
      this.#contract.workspaceId,
    )) {
      try {
        if (this.#featureDeliveryRecovery === undefined) {
          throw new Error('prepared feature delivery requires a configured recovery driver');
        }
        await this.#featureDeliveryRecovery.reconcileFinalizing(runId);
        if (this.#store.listPreparedFeatureDeliveryOperations(runId).length > 0) {
          throw new Error('feature delivery recovery left a prepared effect behind');
        }
      } catch (error) {
        failures.push(error);
      }
    ...
    if (failures.length > 0) {
      throw new AggregateError(failures, 'one or more workflow effects require recovery');
    }
    return reconciled;
  }
      ...
      input.to === 'recovering'
        ? (this.#store.getLatestCommittedTransitionInto(input.execution.runId, input.from) ??
          (input.from === 'finalizing'
            ? (this.#store.getCommittedFeatureDeliveryAttestation(input.execution.runId) ??
      ...
      externalArguments: {
        ...
        ...(input.to === 'recovering'
          ? {
              ...
              'kind' in interrupted &&
              interrupted.kind === 'feature.github.merge'
                ? {
                    featureContractDigest: (interrupted.request as Record<string, unknown>)
                      .featureContractDigest,
                    featureDeliveryAttestationDigest: `sha256:${createHash('sha256')
                      .update(JSON.stringify(interrupted.result))
                      .digest('hex')}`,
                  }
                : {}),
            }
