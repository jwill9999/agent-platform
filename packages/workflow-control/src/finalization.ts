...
import { featureMergeRequestSchema } from './featureDeliveryBroker.js';
import {
...
interface CommittedFeatureDelivery {
  operationId: string;
...
export class FeatureFinalizationCoordinator {
  ...
  async finalize(input: {
    ...
    const featureDelivery = this.#assertFeatureDeliveryCommitted(input.runId);
    if (this.#store.listPreparedFeatureDeliveryOperations(input.runId).length > 0) {
      throw new Error('feature closeout requires prepared feature delivery recovery first');
    }
  ...
  #buildReport(runId: string, epicId: string): FeatureFinalReport {
    ...
    const featureDelivery = this.#assertFeatureDeliveryCommitted(runId);
    const mergedHeadSha = featureDelivery?.integratedHeadSha ?? request?.headSha;
    ...
    return featureFinalReportSchema.parse({
      ...
      destination: featureDelivery?.destination ?? this.#contract.authority.deliveryTarget,
      mergedHeadSha,
      mergeSha: featureDelivery?.mergeSha ?? result?.mergeSha,
      mergeEventIdentity: featureDelivery?.eventIdentity ?? result?.eventIdentity,
  ...
  #assertFeatureDeliveryCommitted(runId: string): CommittedFeatureDelivery | undefined {
    const durableContract = this.#store.getApprovedFeatureDeliveryContract(runId);
    if (durableContract === undefined) return undefined;
    const contract = featureDeliveryContractSchema.parse(durableContract);
    const executionDigest = `sha256:${createHash('sha256')
      .update(JSON.stringify(this.#contract))
      .digest('hex')}`;
    if (
      contract.executionContractDigest !== executionDigest ||
      contract.featureId !== this.#contract.featureId ||
      contract.workspaceId !== this.#contract.workspaceId ||
      contract.policyDigest !== this.#contract.policyDigest ||
      contract.origin.executionContractVersion !== this.#contract.contractVersion ||
      contract.origin.repository !== this.#contract.authority.github.repository
    ) {
      throw new Error('feature closeout contract differs from its immutable execution origin');
    }
    const contractDigest = deriveFeatureDeliveryContractDigest(contract);
    this.#store.assertRunUsesFeatureDeliveryContract(runId, contractDigest, contract);
    const delivery = this.#store.getCommittedFeatureDeliveryAttestation(runId);
    if (delivery === undefined) {
      throw new Error('feature closeout requires a committed staging delivery attestation');
    }
    const request = featureMergeRequestSchema.parse(delivery.request);
    const result = z
      .object({
    ...
    if (
      request.featureContractDigest !== contractDigest ||
      request.workspaceId !== contract.workspaceId ||
      request.taskId !== contract.origin.taskId ||
      request.repository !== contract.authority.repository ||
      request.originMergeOperationId !== contract.origin.integrationMergeOperationId ||
      request.originMergeAttestationDigest !== contract.origin.integrationMergeAttestationDigest ||
      request.originPullRequestNumber !== contract.origin.integrationPullRequestNumber ||
      request.originTaskHeadSha !== contract.origin.taskHeadSha ||
      request.headRef !== contract.authority.headRef ||
      request.headSha !== contract.authority.headSha ||
      request.base !== contract.authority.base ||
      request.protectionDigest !== contract.authority.protectionDigest ||
      request.mergeMethod !== contract.authority.mergeMethod ||
      request.adminBypass !== contract.authority.adminBypass ||
      result.pullRequestNumber !== request.pullRequestNumber ||
      result.headSha !== request.headSha ||
      result.base !== request.base ||
      result.mergeMethod !== request.mergeMethod
    ) {
      throw new Error('feature closeout staging delivery attestation differs from its contract');
    }
    return {
      operationId: delivery.id,
      contractDigest,
      integratedHeadSha: result.headSha,
      destination: result.base,
